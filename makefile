...
program_LIBRARIES := rt pthread nvidia-ml 
...
GEN_SM21 := -arch=sm_21 #Using -gencode requires 2.0, not 2.1. Compiling with 2.0 this way uses too many resources for Tesla.
NVFLAGS := -O3 
NVFLAGS += $(GEN_SM35) $(GEN_SM21)
...
debug: CXXFLAGS = -g -O0 -std=c++0x -Wall -pedantic $(EXTRA_FLAGS)
debug: NVFLAGS = -g -G -O0 -arch=sm_21 $(EXTRA_FLAGS)
debug: $(BC)

debugv: CXXFLAGS = -g -O0 -std=c++0x -Wall -pedantic $(EXTRA_FLAGS)
debugv: NVFLAGS = -g -G -O0 --ptxas-options="-v" -arch=sm_21 $(EXTRA_FLAGS)
debugv: $(BC)
